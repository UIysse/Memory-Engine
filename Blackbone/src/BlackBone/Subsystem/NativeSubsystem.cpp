...
#include "../../../../QtPro/DebuggedProcess.h"
...
ptr_t Native::getPEB( _PEB64* ppeb )
{
    ...
    ULONG bytes = 0;
	DRIVOUT << "non wow64 peb ft call" << std::endl;
	...
	{
		DRIVOUT << "ntQueryInformationProcess succeded." << std::endl;
		if (ReadProcessMemory(_hProcess, pbi.PebBaseAddress, ppeb, sizeof(_PEB32), NULL) == 0)
			DRIVOUT << "WinAPI readProcMem failed " << std::endl;
		else
		{
			DRIVOUT << "WinAPI readProcMEM succeded to retrieve PEB info" << std::endl;
			DRIVOUT << " pbi.PebBaseAddress : 0x" << pbi.PebBaseAddress << " Mutant : 0x " << std::hex << ppeb->Mutant << " ImageBaseAddress  : 0x" << std::hex << ppeb->ImageBaseAddress << std::endl;
			SHORT MZ;
			DebuggedProc.hwnd = _hProcess;
			ReadProcessMemory(DebuggedProc.hwnd, (LPCVOID)ppeb->ImageBaseAddress, &MZ, sizeof(MZ), NULL);
			DRIVOUT << " pbi.PebBaseAddress : 0x" << std::hex << MZ << std::endl;
			//###
			using namespace std;
			HMODULE *modarray = new HMODULE[200];
			DWORD o;
			if (0 == EnumProcessModules(DebuggedProc.hwnd, modarray, sizeof(HMODULE) * 200, &o))
				DRIVOUT << "Enum Process failed" << endl;
			MODULEINFO  lpmod;
			if (0 == GetModuleInformation(DebuggedProc.hwnd,
				...
				DRIVOUT << "GetModuleInformation failed." << endl;
			DebuggedProc.addressOfInterest = reinterpret_cast<int64_t>(lpmod.EntryPoint);
			DRIVOUT << "Entrypoint : " << hex << DebuggedProc.addressOfInterest << endl;
		}
	}
	else
		DRIVOUT << "ntQueryInformationProcess failed" << std::endl;
